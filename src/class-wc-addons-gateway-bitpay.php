...
/**
 * Bitpay Gateway for subscriptions
 *
 * @class 		WC_Addons_Gateway_Bitpay
 * @extends		WC_Gateway_Bitpay
 * @since       2.2.0
 * @version		1.0.0
 * @package		WooCommerce/Classes/Payment
 * @author 		WooThemes
 */
class WC_Addons_Gateway_Bitpay extends WC_Gateway_Bitpay {
	...
	public function __construct() {
		...
		add_filter( 'woocommerce_bitpay_hosted_args', array( $this, 'hosted_payment_args' ), 10, 2 );
		add_action( 'woocommerce_api_wc_addons_gateway_bitpay', array( $this, 'return_handler' ) );
	}
	...
	public function hosted_payment_args( $args, $order_id ) {
		...
		$args['redirect-url'] = WC()->api_request_url( 'WC_Addons_Gateway_Bitpay' );
	...
	protected function process_subscription( $order, $cart_token = '' ) {
		try {
			if ( empty( $cart_token ) ) {
				...
				throw new Bitpay_ApiException( $error_msg );
			}

			// Create customer
			$customer = Bitpay_Customer::createCustomer( array(
				'token'     => $cart_token,
			...
			if ( is_object( $customer ) && '' != $customer->id ) {
				...
				update_post_meta( $order->id, '_bitpay_customer_id', $customer_id );
			} else {
				$error_msg = __( 'Error creating user in Bitpay.', 'woocommerce' );
		...
		} catch ( Bitpay_ApiException $e ) {
			if ( $e instanceof Bitpay_BadRequestException && $e->hasFieldErrors() && $e->getFieldErrors() ) {
	...
	public function process_payment( $order_id ) {
		$cart_token = isset( $_POST['bitpay_token'] ) ? wc_clean( $_POST['bitpay_token'] ) : '';
	...
	public function process_subscription_payment( $order = '', $amount = 0 ) {
		...
		if ( $amount * 100 < 50 ) {
			return new WP_Error( 'bitpay_error', __( 'Sorry, the minimum allowed order total is 0.50 to use this payment method.', 'woocommerce' ) );
		}

		$customer_id = get_post_meta( $order->id, '_bitpay_customer_id', true );

		if ( ! $customer_id ) {
			return new WP_Error( 'bitpay_error', __( 'Customer not found', 'woocommerce' ) );
		}

		try {
			// Charge the customer
			$payment = Bitpay_Payment::createPayment( array(
			...
			return new WP_Error( 'bitpay_payment_declined', $e->getMessage(), array( 'status' => $e->getCode() ) );
		}
		...
		if ( 'APPROVED' == $payment->paymentStatus ) {
			...
			return new WP_Error( 'bitpay_payment_declined', __( 'Payment was declined - please try another card.', 'woocommerce' ) );
		}
	}
	...
	public function remove_renewal_order_meta( $order_meta_query, $original_order_id, $renewal_order_id, $new_order_role ) {
		if ( 'parent' == $new_order_role ) {
			$order_meta_query .= " AND `meta_key` NOT LIKE '_bitpay_customer_id' ";
		}
	...
	public function update_failing_payment_method( $original_order, $renewal_order, $subscription_key ) {
		$new_customer_id = get_post_meta( $renewal_order->id, '_bitpay_customer_id', true );

		update_post_meta( $original_order->id, '_bitpay_customer_id', $new_customer_id );
	}
	...
	public function process_pre_order_release_payment( $order ) {
		...
		try {
			...
			if ( 'APPROVED' == $payment->paymentStatus ) {
				...
				return new WP_Error( 'bitpay_payment_declined', __( 'Payment was declined - the customer need to try another card.', 'woocommerce' ) );
			}
		} catch ( Exception $e ) {
		...
		if ( isset( $_REQUEST['reference'] ) && isset( $_REQUEST['amount'] ) ) {
			...
			if ( $amount === $order_total ) {
				...
				if ( 'success' == $response['result'] ) {
					...
					$order->update_status( 'failed', __( 'Payment was declined by Bitpay.', 'woocommerce' ) );
				}
